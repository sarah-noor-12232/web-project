...
const LoginSignup = () => {
  ...
  const handleSubmit = (event) => {
    ...
    navigate('/user-auth/customer/Catalogue');
  };

  // Handle navigation to the vendor dashboard
  const handleVendorLogin = (event) => {
    event.preventDefault();
    
    navigate('/user-auth/vendor/dashboard'); // Navigate to the vendor dashboard route
  };
            ...
            <div className="flip-card__inner">
              {isLogin ? (
                <div className="flip-card__front">
                  <div className="title01">Log in</div>
                  <form onSubmit={handleSubmit} className="flip-card__form">
                    <input type="email" placeholder="Email" name="email" value={formData.email} onChange={handleChange} className="flip-card__input" />
                    <input type="password" placeholder="Password" name="password" value={formData.password} onChange={handleChange} className="flip-card__input" />
                    <div className="log_buttons">
                    <button type="button" className="flip-card__btn2" onClick={handleVendorLogin}>Log In as Vendor</button>
                      <button type="submit" className="flip-card__btn2" onClick={handleSubmit}>Log In as Renter</button>
                    </div>
                  </form>
                </div>
              ) : (
                <div className="flip-card__back">
                  <div className="title01">Sign up</div>
                  <form onSubmit={handleSubmit} className="flip-card__form">
                    <input type="text" placeholder="Name" name="name" value={formData.name} onChange={handleChange} className="flip-card__input" />
                    <input type="email" placeholder="Email" name="email" value={formData.email} onChange={handleChange} className="flip-card__input" />
                    <input type="password" placeholder="Password" name="password" value={formData.password} onChange={handleChange} className="flip-card__input" />
                    <div className="form-group">
                      <label className="radio-inline mr-3">
                        <input type="radio" name="userType" value="renter" checked={formData.userType === 'renter'} onChange={handleChange} /> Renter
                      </label>
                      <label className="radio-inline">
                        <input type="radio" name="userType" value="vendor" checked={formData.userType === 'vendor'} onChange={handleChange} /> Vendor
                      </label>
                    </div>
